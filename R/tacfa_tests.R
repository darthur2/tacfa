...
system.time(replicate(1000, comp_Z_post_R(W, FF, Gamma, Psi, delta)))
system.time(replicate(1000, comp_Z_post_cpp(W, FF, Gamma, Psi, delta)))
...
p <- rep(1/K, K)
samples_R <- sample(1:K, 1000, TRUE, p)
samples_cpp <- replicate(1000, sample_cpp(p)) + 1

table(samples_R)
table(samples_cpp)

system.time(replicate(10000, sample(1:K, 1, TRUE, p)))
system.time(replicate(10000, sample_cpp(p) + 1))

### Test rmvnorm Function ###

Mu <- c(1, 3, -3)
Sigma <- rWishart(1, 3, diag(3))[,,1]

mv_samps_R <- rmvnorm_R(1000, Mu, Sigma)
mv_samps_cpp <- rmvnorm_cpp(1000, Mu, Sigma)

Mu
colMeans(mv_samps_R)
colMeans(mv_samps_cpp)

Sigma
cov(mv_samps_R)
cov(mv_samps_cpp)

system.time(replicate(10000, rmvnorm_R(100, Mu, Sigma)))
system.time(replicate(10000, rmvnorm_cpp(100, Mu, Sigma)))
...
draws_R <- rtnorm_R(n, mu_trunc, sigma_trunc, lower, upper)
draws_cpp <- rtnorm_cpp(n, mu_trunc, sigma_trunc, lower, upper)
...
system.time(replicate(10000, rtnorm_R(100, mu_trunc, sigma_trunc, lower, upper)))
system.time(replicate(10000, rtnorm_cpp(100, mu_trunc, sigma_trunc, lower, upper)))
